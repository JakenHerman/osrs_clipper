...
Our goal is to build an OSRS RuneScape clipping tool that does the following:
 ...
 - [x] Generate Highlight Clips: When a key event is detected, automatically generate a video clip from the stream using tools like FFmpeg.

 - [x] Store Clips on S3: Upload the generated clip to an Amazon S3 bucket for persistent storage.

 - [x] Provide a Web API: Create an API using Actix Web to retrieve a list of clips and provide access to each clip’s URL.
